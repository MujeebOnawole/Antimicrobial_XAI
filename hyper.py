...
class HyperparameterOptimization:
    def __init__(self, config: Configuration, n_trials: int = 50):
        ...
        if self.config.classification:
            ...
            self.optimization_metric = 'val_rmse'
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            metric_value = trainer.callback_metrics.get(self.optimization_metric)  # Gets val_auc or val_rmse
            if metric_value is None:
                if self.config.classification:
                    metric_value = trainer.callback_metrics.get('val_auc')
                else:
                    metric_value = trainer.callback_metrics.get('val_rmse')
    ...
    def save_results(self, study: optuna.Study) -> None:
        try:
            ...
            if self.config.classification:
                ...
                essential_metrics = ['val_loss', 'val_rmse']  # Match model.py validation_step
                metric_format = {
                    'val_loss': '.6f',
                    'val_rmse': '.6f'
                }
            ...
            try:
                ...
                if len(results) > 0:
                    if self.config.classification:
                        ...
                        best_trial = min(
                            (r for r in results if r['val_rmse'] != 'N/A'), 
                            key=lambda x: float(x['val_rmse']), 
                            default=None
                        )
                    if best_trial:
                        metric = 'val_auc' if self.config.classification else 'val_rmse'
        ...
        try:
            ...
            if self.config.classification:
                ...
                if checkpoint_metric is None:
                    ...
                    retraining_reasons.append(
                        f"Metric value mismatch for '{metric_name}': Checkpoint={checkpoint_metric}, Best trial={best_metric}, Tolerance={tolerance}"
                    )
                    needs_retraining = True
